...
### 1) 참여 방법
- 선생님(또는 진행자)이 공유한 **PARTICIPANT 링크**로 접속합니다.
- 화면에 보이는 선택지 중 **가장 적절한 1개**를 고릅니다.
- 선택한 이유를 **짧게라도 직접 작성**한 뒤 제출합니다.
- 제출 후에는 대기 화면이 보이며, 결과는 세션 종료 후 공개됩니다.

### 2) 유의사항
- 한 번 제출한 응답은 같은 세션에서 다시 제출할 수 없습니다.
- 이유 작성 시 욕설/비방/개인 신상정보 입력은 피해주세요.
- 네트워크가 불안정하면 제출 버튼을 여러 번 누르지 말고 잠시 기다려 주세요.
- 진행 중인 세션에서는 실시간 결과가 공개되지 않습니다.

### 3) 개인정보 미수집 안내
- 이 설문은 이름, 학번, 전화번호, 이메일 등 **직접 식별 가능한 개인정보를 수집하지 않습니다.**
- 시스템에는 설문 응답 처리에 필요한 값(세션 식별값, 선택지, 작성한 이유, 제출 시각)만 저장됩니다.
- 작성한 이유에 개인을 특정할 수 있는 내용은 입력하지 않도록 주의해 주세요.
